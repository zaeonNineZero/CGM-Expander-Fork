 ...
 */
public class RifleCustomModel implements IOverrideModel
{
    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType, ItemStack stack, ItemStack parent, @Nullable LivingEntity entity, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay)
    {
        ...
        BakedModel bakedModel = ExpandedModelComponents.RIFLE_BASE.getModel();
        if (getVariant(stack, "BaseVariant") == 1)
        bakedModel = ExpandedModelComponents.RIFLE_BASE_1.getModel();
		...
		{
            RenderUtil.renderModel(ExpandedModelComponents.RIFLE_SIGHTS.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
		}
        ...
        RenderUtil.renderModel(ExpandedModelComponents.RIFLE_BOLT.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
		...
		// Render the transformed model.
        ExpandedModelComponents magModel = ExpandedModelComponents.RIFLE_MAGAZINE;
        try {
            ...
            if(!magStack.isEmpty())
            {
	            if (magStack.getItem().builtInRegistryHolder().key().location().getPath().equals("light_magazine"))
		    		magModel = ExpandedModelComponents.RIFLE_LIGHT_MAG;
	            else
	            if (magStack.getItem().builtInRegistryHolder().key().location().getPath().equals("extended_magazine"))
			    	magModel = ExpandedModelComponents.RIFLE_EXTENDED_MAG;
            }
		}
